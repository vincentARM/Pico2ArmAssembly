...
.equ NVIC_ICER_OFFSET, 0x0000e180
...
udpd_ctrl:                                         @ In + out 
    .struct  udpd_ctrl + 8 * (USB_NUM_ENDPOINTS - 1)       @ 8* 15 = 120 = 0x78 + 8 = 0x80
udpd_buf_ctrl:                                     @ In + out 
    .struct  udpd_buf_ctrl + 8 * (USB_NUM_ENDPOINTS)
udpd_ep0_buf_a:                                    @ 
    .struct  udpd_ep0_buf_a + TAILLEPAQUET    @  0x40
udpd_ep0_buf_b:                                    @ 
    .struct  udpd_ep0_buf_b + TAILLEPAQUET                  @  0x40
udpd_epx_data:                                     @ 
    .struct  udpd_epx_data + ( USB_DPRAM_SIZE - 0x400)          @ 
udpd_fin:
    ...
    .equ LGCFGENDPOINT, . - cfg_endpoints
            ...
            // 3ième nouveau
             ...
             // 4ième nouveau
                        ...
                        .int endpoint2IN
                        .int ep2InHandler 
                        .int USBCTRL_DPRAM_BASE+udpd_ctrl + 0x8   @ in poste 2 IN Modif
                        .int USBCTRL_DPRAM_BASE+udpd_buf_ctrl+0x10 @ in poste 2 IN
                        .int USBCTRL_DPRAM_BASE+udpd_epx_data+TAILLEPAQUET
                        .int 0 
             // 5ième nouveau
                        .int endpoint2
                        .int ep2OutHandler
                        .int USBCTRL_DPRAM_BASE+udpd_ctrl + 0xC  @ out poste 1 OUT Modif
                        .int USBCTRL_DPRAM_BASE+udpd_buf_ctrl+0x14 @ in poste 2 OUT
                        .int USBCTRL_DPRAM_BASE+udpd_epx_data+TAILLEPAQUET * 2 @
                        .int 0

             // fin
                        .fill LGCFGENDPOINT * 28,1,0 
sComplement3:           .fill 1000,1,0
.align 4
stLine_info:
                        .int 115200        //
                        .byte 1
...
/**********************************************/
.text
.global initUsbDevice,envoyerMessage,recevoirMessage,isrIrq14,envoyerCar,recevoirCarMod
.thumb_func
loop:
     b loop
/**********************************************/
/* initialisation connexion USB                              */
/**********************************************/
.thumb_func
...
1:
    ...
    ldr r3,iAdrClocks                @ initialisation horloge USB
    movs r1,1                        @ obligatoire pour le fonctionnement
    lsls r1,11                       @ pour 0x800
    str  r1, [r3,#CLK_USB_CTRL]
  ...
  ///  bl ledEclats
    ...
    movs r0,USBCTRL_IRQ+16         @ N° du poste IRQ USB dans la table des vecteurs
...
/******************************************************************/
/*  r0 pointeur uec r1 buffer r2 longueur  r3 top message                */
.thumb_func
...
1:
    ...
    movs r4,64
    subs r4,r2,r4
    movs r2,64                @ envoi des 64 premiers caractères 
    adds r5,r1,r2
    movs r6,r0
    movs r0,r6
    bl usbTransfert
    movs r0,1                  @ attente obligatoire !!!
    bl attendre
    cmp r3,#1                @ 
    bne 2f                   @ 
    cmp r4,64                @ taille maxi d'un paquet ?
    bge 2f  
    movs r0,EP2_IN_ADDR      @ ne pas envoyer tous les 64 octets
...
recevoirMessage:                    @ INFO: recevoirMessage
    ...
    ldr r1,iAdriTopCar
    mov r2,0
    str r2,[r1]         @ raz top saisie 1 caractere
    movs r2,0
...
.align 2
/******************************************************************/
/*     lecture un caractere du host                                */ 
/******************************************************************/
/*  INFO:  cette fonction ne semble pas utilisée */
/* r0 adresse buffer reception */
/*  r1 delai attente */
.thumb_func
...
.align 2
/******************************************************************/
/*     lecture des caracteres du host  pour xmodem                                     */ 
/******************************************************************/
/*  r0 delai attente                 */
/*  r1 adresse buffer */
.thumb_func
recevoirCarMod:                    @ INFO: recevoirCarMod
    ...
    mov r4,r0
    mov r5,r1
    ldr r2,iAdriTopCar  @ positionne ce top pour epe2Outhandler
    mov r1,1
    str r1,[r2]         @ top reception des caractere
    ldr r3,iAdriTopSaisieOk
    movs r2,0
    str r2,[r3]         @ raz top reception
    
1:                      @ boucle attente
    ldr r2,[r3]         @ lecture top saisie
    cmp r2,1
    beq  2f             @ l envoi est terminé
    movs r0,1           @ sinon on attend : attention important
    bl attendre
    subs r4,r4,1
    blt 4f
    b 1b
2:                      @ extrait nombre de caractere
    ldr r6,iAdriCptCarSaisi
    ldr r6,[r6]
    ldr r1,iAdrsBufferRec
    mov r2,0
3:
    ldrb r0,[r1,r2]        @ copie les caracteres dans le buffer
    strb r0,[r5,r2]
    add r2,r2,1
    cmp r2,r6           @ nombre de caracteres
    blt 3b
    ldrb r5,[r1]        @ charge le premier caractere lu
    
    movs r0,EP2_IN_ADDR  @ pour indiquer que le paquet reçu est traité
    bl usbgetendpointconfiguration 
    movs r1,0
    movs r2,0
    movs r3,1
    ...
    mov r0,r5           @ premier caractere
    mov r1,r6           @ nb de caractere
    
    b 100f
4:
    mov r0,-1           @ delai attente depassé    
100:
    pop {r2-r6,pc}
.align 2
iAdriTopCar:      .int iTopCar
/******************************************************************/
...
envoyerCar:                  @ INFO: envoyerCar
    push {r0-r4,lr}
    sub sp,sp,4
    mov r1,sp                @ adresse du caractere
    str r0,[r1]              @ stocke le caractere en memoire
    ldr r0,iAdrcfg_endpoints
    add r0,r0,uec_fin * 4
    mov r4,r0
    mov r2,1                 @ longueur
    mov r3,1                 @  top message
    bl usbstarttransfert
    movs r0,1                @ 
    bl attendre
    mov r0,r4
...
100:
    ...
    pop {r0-r4,pc}
.align 2
iAdrcfg_endpoints:     .int cfg_endpoints
/******************************************************************/
/*     gestion endpoint 0    envoi vers host                                    */ 
/******************************************************************/
...
/*                    */
.thumb_func
ep4InHandler:                    @ INFO: ep2InHandler
    push {lr}
    movs r1,r0
    movs r2,r1
...
ep2OutHandler:             @ INFO: ep2OutHandler
    ...
    ldr r1,iAdriTopCar
    ldr r1,[r1]
    cmp r1,1                 @ attente reception xmodem
    beq 4f 
...
1:
    cmp r0,0x7f              @ car = del ? 
    bne 2f
    cmp r3,#0             @ pas de caractère déjà saisi ?
    beq 100f
    subs r3,1             @ retour arrière
...
3:
    ...
    bl usbstarttransfert
    b 100f
4:                          @ traitement envoi xmodem
   mov r2,0
   ldr r1,iAdrsBufferRec    @ charge l adresse buffer
5:
   ldrb r0,[r4,r2]          @ charge le caractère
   strb r0,[r1,r2]          @ stocke le caractère reçu
   add r2,r2,1
   cmp r2,r5                @ nombre de caractere reçus
   blt 5b
   ldr r1,iAdriTopSaisieOk
   movs r0,1
   str r0,[r1]

   ldr r0,iAdriCptCarSaisi  @ charge adresse du compteur de caractère
   str r5,[r0]              @ et stocke le nombre de caractere reçu
...
ep2InHandler:             @ INFO: ep2InHandler
    ...
    bl usbgetendpointconfiguration 
    movs r1,0
    movs r2,TAILLEPAQUET            @ obligatoire
    movs r3,0
    bl usbstarttransfert
    pop {pc}
.align 2
...
usbClassCDC:                         @ INFO: usbClassCDC
   ...
   // bl ledEclats
    ...
    movs r0,2
    bl attendre
    movs r0,EP0_IN_ADDR               @ ajout 2022
    bl usbgetendpointconfiguration
    movs r1,0
    movs r2,0
    movs r3,0
...
4:
   ...
   // bl ledEclats
    ...
    movs r2,line_fin
    movs r3,0
    bl usbstarttransfert         @ aucun effet !!!
    movs r0,3
    bl ledEclats
    //movs r0,2
    //bl attendre
...
7:
   ...
   // mov r0,2
   // bl ledEclats
    movs r0,EP2_IN_ADDR     @ oui envoi message début
    bl usbgetendpointconfiguration
    movs r1,0
    movs r2,0               @  OK  
    movs r3,0
